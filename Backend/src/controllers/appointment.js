...
export const createAppointment = async (req, res, next) => {
    try {
        ...
        if (appointment) {
            res.status(200).json({ status: 1, success: true, message: "Appointment created", appointment })
        } else {
...
export const getAllAppointment = async (req, res, next) => {
    try {
        ...
        if (appointments) {
            ...
            res.status(200).json({ status: 1, success: true, message: "Appointment details fetched", appointment: formattedAppointment })
        } else {
...
export const getAppointmentById = async (req, res, next) => {
    try {
        ...
        if (appointments) {
            ...
            res.status(200).json({ status: 1, success: true, message: "Appointments fetched.", appointment: formattedAppointment })
        } else {
            return next(new ErrorHandler("Cannot fetch.", 400))
        }
    } catch (error) {
...
export const deleteAppointment = async (req, res, next) => {
    try {
        ...
        if (deleteAppointment) {
            res.status(200).json({ status: 1, success: true, message: "Appointment deleted." })
        } else {
...
export const updateAppointmentStatus = async (req, res, next) => {
    try {
        ...
        res.status(200).json({ status: 1, success: true, message: "Appointment updated." })
    } catch (error) {
        console.log(error)
        res.status(500).json({ error: error })
    }
