...
export const createBrand = async (req, res, next) => {
    try {
        ...
        if (validate.length === 0) {
            ...
            if (newBrand) {
                res.status(201).json({
                    status: 1, success: true, message: "Brand created.", newBrand,
                });
            }
        } else {
...
export const getAllBrands = async (req, res) => {
    try {
        ...
        res.status(201).json({ status: 1, success: true, message: "Brand details fetched.", brandsData });
    } catch (error) {
        console.log(error);
        res.status(500).json({ status: 0, message: "Error", error: error });
    }
...
export const deleteBrandById = async (req, res, next) => {
    try {
        ...
        if (deleteBrand) {
            return res.status(201).json({ status: 1, success: true, message: "Brand deleted", })
        } else {
...
export const updateBrandById = async (req, res, next) => {
    try {
        ...
        const updateBrand = await Brand.findByIdAndUpdate(id, updateFields, { new: true })
        if (updateBrand) {
            return res.status(201).json({ status: 1, success: true, message: "Brand updated", updateBrand })
        } else {
